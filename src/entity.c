...
static int utf8_char_width(const char *s, int seq_len) {
...
static bool seaweed_reached_limit(const struct entity *e) {
  if (e->seaweed_orig_height + e->seaweed_grown >= e->seaweed_orig_height * 2) return true;
  return e->y <= 5.0;
}
...
static void tick_seaweed_growth(struct entity *e) {
  if (e->seaweed_capped) {
...
static void tick_seaweed_debris(struct entity *e, int term_h) {
  ...
  entity_clear_owned(e);
  char *tilde = xmalloc(2);
  memcpy(tilde, "~", 2);
  entity_set_owned_single_row(e, tilde, 0.0);
  e->die_after = rng_double(10.0) + 10.0;
}
