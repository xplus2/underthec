...
static unsigned utf8_decode(const char *s, int seq_len) {
  ...
  unsigned cp = (seq_len == 2) ? (unsigned)(c0 & 0x1F) : (seq_len == 3) ? (unsigned)(c0 & 0x0F) : (unsigned)(c0 & 0x07);
  for (int k = 1; k < seq_len; k++) cp = (cp << 6) | (unsigned)((unsigned char)s[k] & 0x3F);
  return cp;
}

/* most common cases: cjk, hangul, emoji blocks */
struct wide_range { unsigned lo, hi; };
...
static bool codepoint_is_wide(unsigned cp) {
  int lo = 0, hi = (int)(sizeof(wide_ranges) / sizeof(wide_ranges[0])) - 1;
  while (lo <= hi) {
    int mid = (lo + hi) / 2;
    ...
    for (int row = 0; rows[row] != NULL; row++) {
      ...
      for (int byte_idx = 0; byte_idx < byte_len; ) {
        ...
        if (!cell_transparent(e, srow, byte_len, byte_idx)) {
          ...
          canvas_put(c, base_x + col, base_y + row, srow + byte_idx, seq_len, a, width);
        }
        byte_idx += seq_len;
        col += width;
      }
